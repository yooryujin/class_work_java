...
public class MyBookStore {
    ...
    public static void main(String[] args) {
        ...
        while (flag) {
            ...
            // 숫자 비교할 때 == 사용한다.
            // 문자열 비교할 때 "문자열".equals("문자열") --> true, 문자열 내용이 다르다면 false 반환한다.
    ...
    public static void readByTitle(Scanner scanner, Book[] books) {
        ...
        // 문자열 기능 중에 공백 제거 기능이 있다. .trim(); <-- 앞뒤 공백을 제거
        String title = scanner.nextLine();
        ...
        for (int i = 0; i < books.length; i++) {
            //"홍길동".equals
            // 6번째 인덱스의 값은 null 가리키고 있다 - 오류발생
            // 방어적 코드 작성
            if (books[i] != null) {
                // title.trim() <-- 공백 제거. 문자 중간에 있는 공백은 제거되지 않음
    ...
    // 전체 삭제 하기 - 배열 안에 담겨 있는 Book() 정보 null 처리
    public static void deleteAll(Book[] books) {
        System.out.println("--- deleteAll() ---");
        // 1. 배열 주소를 가져와서 반복문 100 = null;
        for (int i = 0; i < books.length; i++) {
            // 전체 삭제 중
            // books = null; // 메모리공간까지 삭제됨
            // books[0] ~ books[100] = null;
            books[i] = null;
        }
    }
